...
class Config():
    ...
    # training
    _checkpoint_folder = './checkpoints/tracknet_v2_mse_360_640_6f_pre'
    _batch_size = 2
    _epochs = 10

    def get_model(self):
        model = TrackNetV2MSE(sequence_length=self._sequence_length, one_output_frame=self._one_output_frame)
        model.vgg_conv1[0][0] = torch.nn.Conv2d(9, 64, kernel_size=(3, 3), stride=(1, 1), padding='same')
        model.load('./checkpoints/tracknet_v2_mse_360_640/checkpoint_0027_best.ckpt')
        model.vgg_conv1[0][0] = torch.nn.Conv2d(3*self._sequence_length, 64, kernel_size=(3, 3), stride=(1, 1), padding='same')
        return model
