...
def launch_training(device=None):
    ...
    phase_procedures = [
        {'epochs': 10, 'phase': 'phase_1_0', 'lr': 1e-3, 'tc': 0.8, 'c': 0.1,   'gt': 1   },
        {'epochs': 5 , 'phase': 'phase_1_1', 'lr': 1e-3, 'tc': 0,   'c': 0.5,   'gt': 0.8 },
        {'epochs': 5 , 'phase': 'phase_2_0', 'lr': 1e-4, 'tc': 0,   'c': 1/3,   'gt': 1/2 },
        {'epochs': 5 , 'phase': 'phase_2_1', 'lr': 1e-4, 'tc': 0,   'c': 1/5,   'gt': 1/5 },
        {'epochs': 5 , 'phase': 'phase_3_0', 'lr': 1e-5, 'tc': 0,   'c': 1/30,  'gt': 1/30},
        {'epochs': 5 , 'phase': 'phase_3_1', 'lr': 1e-5, 'tc': 0,   'c': 1/200, 'gt': 0   },
    ]

    optimizer = Adam(model.parameters())

    for i, phase in enumerate(phase_procedures):
        checkpoint_folder = os.path.join(config._checkpoint_folder, phase['phase'])
        ...
        collate_fn = partial(collate_fn_rnn,
                            total_clear_probability=phase['tc'],
                            clear_probability=phase['c'],
                            ground_truth_probability=phase['gt'],
                            sequence_length=config._sequence_length)
